...
def _(pd):
    ...
    print(f"{len(df)} chemicals across {df['class'].nunique()} classes.")
    df.head()
    return (df,)
...
def _(make_axis, model):
    ...
    axis_reactivity = make_axis(axis1_pos, axis1_neg, model)
    return (axis_reactivity,)
...
def _(make_axis, model):
    ...
    axis_biological = make_axis(axis2_pos, axis2_neg, model)
    return (axis_biological,)
...
@app.cell
def _(axis_biological, axis_reactivity, df, model):
    x = score_words(df["name"].tolist(), axis_reactivity, model)
    y = score_words(df["name"].tolist(), axis_biological, model)
...
@app.cell(hide_code=True)
def _(alt, df_scored, mo):
    class_colors = alt.Color(
        "class:N",
        legend=alt.Legend(title="Chemical Class"),
        scale=alt.Scale(scheme="tableau20"),
    )

    chart = (
        alt.Chart(df_scored)
        .mark_circle(size=95, opacity=0.82, stroke="white", strokeWidth=0.6)
        .encode(
            x=alt.X(
                "x:Q",
                title="← stable / inert          reactive / energetic →",
                scale=alt.Scale(zero=False, padding=20),
                axis=alt.Axis(grid=False),
            ),
            y=alt.Y(
                "y:Q",
                title="← industrial / inorganic          biological / organic →",
                scale=alt.Scale(zero=False, padding=20),
                axis=alt.Axis(grid=False),
            ),
            color=class_colors,
            tooltip=[
                alt.Tooltip("name:N", title="Chemical"),
                alt.Tooltip("class:N", title="Class"),
                alt.Tooltip("x:Q", title="Reactivity / Energy Score", format=".3f"),
                alt.Tooltip("y:Q", title="Biological / Organic Score", format=".3f"),
            ],
        )
        .properties(
            width=760,
            height=520,
            title="Chemical Classes in a 2D semantic-axis space",
        )
